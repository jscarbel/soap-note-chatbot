...
describe('UserService', () => {
  ...
  describe('searchUsersByName', () => {
    beforeEach(async () => {
      const users = [
        ...
        createTestUser({
          name: 'Jane Doe',
          email: createTestEmail('jane'),
        }),
        createTestUser({
          name: 'Bob Johnson',
          email: createTestEmail('bob'),
        }),
        createTestUser({
          name: 'Alice Johnson',
          email: createTestEmail('alice'),
        }),
      ];
    ...
    it('should find users by partial name match', async () => {
      const results = await userService.searchUsersByName('John');
    ...
    it('should return empty array when no matches found', async () => {
      const results = await userService.searchUsersByName('NonExistent');

      expect(results).toHaveLength(0);
    });

    it('should find users by first name', async () => {
      const results = await userService.searchUsersByName('Jane');
    ...
    it('should find users by last name', async () => {
      const results = await userService.searchUsersByName('Johnson');
  ...
  describe('integration scenarios', () => {
    it('should handle complete user lifecycle', async () => {
      ...
      // Verify in search
      const searchResults = await userService.searchUsersByName('Updated');
